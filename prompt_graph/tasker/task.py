...
from prompt_graph.prompt import GPF, GPF_plus, LightPrompt,HeavyPrompt, Gprompt, GPPTPrompt, DiffPoolPrompt, SAGPoolPrompt, GPromptDenoised
...
class BaseTask:
    ...
    def initialize_lossfn(self):
        ...
        if self.prompt_type == 'denoisedPrompt':
            self.criterion = Gprompt_tuning_loss()
    ...
    def initialize_optimizer(self):
        ...
        elif self.prompt_type in ['denoisedPrompt']:
            self.pg_opi = optim.Adam(self.prompt.parameters(), lr=self.lr, weight_decay=self.wd)
    ...
    def initialize_prompt(self):
        ...
        #     self.prompt = DiffPoolPrompt(self.input_dim, num_clusters=5 ).to(self.device)
            ...
            self.prompt = GPromptDenoised(self.hid_dim).to(self.device)
        elif self.prompt_type == 'MultiGprompt':
            nonlinearity = 'prelu'
