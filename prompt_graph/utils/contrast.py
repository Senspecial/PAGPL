...
def contrastive_loss(x1, x2, temperature=0.1):
    ...
    return loss
def contrastive_loss_1(z1, z2, temperature=0.5):
    z1 = F.normalize(z1, dim=1)
    z2 = F.normalize(z2, dim=1)

    batch_size = z1.size(0)
    z = torch.cat([z1, z2], dim=0)  # [2B, D]
    sim = torch.matmul(z, z.T)      # [2B, 2B] cosine similarity matrix

    # 构造正样本对：i 和 i+batch_size 是正对
    pos_mask = torch.eye(batch_size, device=z.device).bool()
    positives = sim[:batch_size, batch_size:][pos_mask]  # shape: [B]

    # 构造负样本（排除自己）
    logits = sim / temperature
    logits_mask = ~torch.eye(2 * batch_size, dtype=torch.bool, device=z.device)
    logits = logits[logits_mask].view(2 * batch_size, -1)

    labels = torch.arange(batch_size, device=z.device)
    loss = F.cross_entropy(logits[:batch_size], labels) + F.cross_entropy(logits[batch_size:], labels)
    return loss / 2
