...
from prompt_graph.blink.reconstruct_graph import reconstruct_graph
from prompt_graph.lggd.eikonal_features import eikonal_features
...
from prompt_graph.utils.CustomDataset import CustomDataset
...
def get_downstream_task_delegate(args:argparse.Namespace):
    ...
    if args.downstream_task == 'NodeTask':
        ...
        data = apply_graph_ldp(args.dataset_name, data, args.epsilon, args.delta )
        print(data.edge_index.max().item())
        data = reconstruct_graph(data, args.epsilon, args.delta, args.gnn_type, args.pre_train_model_path, args.hid_dim, args.num_layer, args.epochs, args.lr, args.reg_weight, args.gnn_weight)
        print(data.edge_index.max().item())
        #data = eikonal_features(data)
        print(data)
        data = data.to(args.device)
        if args.prompt_type in ['Gprompt','denoisedPrompt', 'All-in-one', 'GPF', 'GPF-plus']:
            graphs_list = load_induced_graph(args.dataset_name, data, args.device)
            print(f"graphs_list[0].x.shape: {graphs_list[0].x.shape}")
        ...
        dataset = [apply_graph_ldp(args.dataset_name, data, args.epsilon, args.delta) for data in dataset]
        dataset = [reconstruct_graph(data, args.epsilon, args.delta, args.gnn_type, args.pre_train_model_path, args.hid_dim, args.num_layer, args.epochs, args.lr, args.reg_weight, args.gnn_weight) for data in dataset]
        #dataset = [eikonal_features(data) for data in dataset]
